        ...
        if (senhas.length === 2) {
            if (senhas[0] !== undefined && senhas[1] !== undefined)
                if (verificarIguais(senhas[0], senhas[1])) {
                    ...
                    erro(motivoErro, senhas[0].parentElement);
                    erro(motivoErro, senhas[1].parentElement);
                }
            senhas = [];
        }
    }
...
function verificarEmail(elemento) {
    if (validator_lib_isEmail__WEBPACK_IMPORTED_MODULE_0___default()(elemento.value))
        return true;
    return false;
}
