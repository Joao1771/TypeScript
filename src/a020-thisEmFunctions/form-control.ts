  ...
  for (let formField of formFields) { // para cada input
    ...
    if (senhas.length === 2) {
      if (senhas[0] !== undefined && senhas[1] !== undefined) 
        if(verificarIguais(senhas[0], senhas[1])){
          motivoErro = "Senhas não coincidem"
          erro(motivoErro,senhas[0].parentElement as HTMLElement)
          erro(motivoErro,senhas[1].parentElement as HTMLElement)
        }
      senhas = []
    }
  }
  if (motivoErro === "") console.log("Formulário Enviado")
}
...
function verificarEmail(elemento: HTMLInputElement): boolean {
  if (isEmail(elemento.value)) return true
  return false
}
